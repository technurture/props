...
const BillingDashboard = () => {
  ...
  useQueueUpdateListener(useCallback(() => {
    console.log('[BillingDashboard] Queue update event received, refreshing stats...');
                ...
                {loading ? (
                  ...
                  <div className="table-responsive table-nowrap">
                    <table className="table table-borderless mb-0">
                      <tbody>
                        {stats.pendingAppointments.filter(invoice => invoice.patientId && invoice.visit?.status === 'in_progress').map((invoice) => {
